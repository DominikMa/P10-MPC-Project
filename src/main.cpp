    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // Transform ptsy and ptsy to car coordinate system
          for (int i = 0; i < ptsx.size(); i++) {
            double ptsx_move = ptsx[i] - px;
            double ptsy_move = ptsy[i] - py;
            ptsx[i] = ptsx_move * cos(-psi) - ptsy_move * sin(-psi);
            ptsy[i] = ptsx_move * sin(-psi) + ptsy_move * cos(-psi);
          }

          // Get Eigen vectors from ptsx and ptsy
          ...
          // Get the coefficients for the poly
          ...
          // Calculate future state because of lag
          ...
          vector<double> ptsy_next;
          double x_step = 3;
          int next_amount = 20;
          for (int i = 1; i < next_amount; ++i) {
            double x = x_step * i;
            ptsx_next.push_back(x);
            ptsy_next.push_back(polyeval(coeffs, x));
          }
          ...
          msgJson["mpc_x"] = mpc_x_results;
          msgJson["mpc_y"] = mpc_y_results;
          msgJson["next_x"] = ptsx_next;
          msgJson["next_y"] = ptsy_next;
